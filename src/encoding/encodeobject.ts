...
type PrefixMapObject = typeof prefixMapObject;
type PrefixMapObjectKey = keyof PrefixMapObject;
type PrefixMapObjectLenKey = keyof PrefixMapObject[PrefixMapObjectKey];
type PrefixMapObjectChar = PrefixMapObject[PrefixMapObjectKey][PrefixMapObjectLenKey];
...
const decodePrefixMapObject = Object.fromEntries(
    Object.entries(prefixMapObject).flatMap(([prefix, map]) =>
        Object.entries(map).map(([len, char]) => [char, { prefix, len: parseInt(len) } as const] as const)
    )
) as Record<PrefixMapObjectChar, { prefix: PrefixMapObjectKey; len: PrefixMapObjectLenKey }>;
/**
...
type PrefixMapNumber = typeof prefixMapNumber;
type PrefixMapNumberKey = keyof PrefixMapNumber;
type PrefixMapNumberLenKey = keyof PrefixMapNumber[PrefixMapNumberKey];
type PrefixMapNumberChar = PrefixMapNumber[PrefixMapNumberKey][PrefixMapNumberLenKey];
...
) as Record<PrefixMapNumberChar, { prefix: PrefixMapNumberKey; len: PrefixMapNumberLenKey }>;

function isValidLength(len: number): len is 1 | 2 | 3 | 4 | 5 {
    return len >= 1 && len <= 5;
}

const ARRAY_MARKER = "_";
const OBJECT_MARKER = "^";
function encodeObjectAsArray(obj: object): string {
...
function decodeObjectFromArray(str: string): object {
 ...
 */
export function encodeAnyArray(obj: any[], safer = false): string {
...
function isDecodeMapConstantKey(key: string): key is keyof typeof decodeMapConstant {
    return key in decodeMapConstant;
}
function isDecodePrefixMapNumberKey(key: string): key is keyof typeof decodePrefixMapNumber {
    return key in decodePrefixMapNumber;
}
function isDecodePrefixMapObjectKey(key: string): key is keyof typeof decodePrefixMapObject {
