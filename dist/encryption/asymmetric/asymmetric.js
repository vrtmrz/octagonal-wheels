 ...
 */
async function encryptUInt8ArrayWithSessionKey(data, aesKey, iv) {
 ...
 */
async function wrapAesKeyWithPublicKey(aesKey, publicKey) {
 ...
 */
async function encryptUInt8Array(data, publicKey) {
 ...
 */
async function unwrapAesKeyWithPrivateKey(wrappedAesKeyBuffer, privateKey) {
    return await webCrypto.subtle.unwrapKey("raw", wrappedAesKeyBuffer, privateKey, {
        name: "RSA-OAEP",
    }, {
        name: "AES-GCM",
        length: 256,
 ...
 */
async function decryptUInt8ArrayWithSessionKey(encryptedDataBuffer, aesKey, iv) {
    return await webCrypto.subtle.decrypt({
        name: "AES-GCM",
        iv: iv,
    }, aesKey, encryptedDataBuffer // Contains both ciphertext and authentication tag
    );
}
/**
 * Decrypts data encrypted with the hybrid method of RSA-OAEP and AES-GCM.
 ...
 */
async function decryptUInt8Array(encryptedInfo, privateKey) {
 ...
 */
async function encryptUInt8ArrayWithPublicKey(data, recipientPublicKey, expectedCurve = DEFAULT_ECDH_CURVE) {
 ...
 */
async function decryptUInt8ArrayWithPrivateKey(encryptedData, privateKey) {
    ...
    if (!ECDH_CURVE_NAMES[curveIndex]) {
        throw new AsymmetricEncryptionArgumentError("The curve index is invalid.");
    }
    const expectedCurve = ECDH_CURVE_NAMES[curveIndex]; // Get the expected curve name from the index
...
export { decryptUInt8Array, decryptUInt8ArrayWithPrivateKey, encryptUInt8Array, encryptUInt8ArrayWithPublicKey, encryptUInt8ArrayWithSessionKey, wrapAesKeyWithPublicKey };
