 ...
 */
const deriveMasterKey = memoWithMap(10, async (passphrase, pbkdf2Salt) => {
    const binaryPassphrase = writeString(passphrase);
    const keyMaterial = await webcrypto.subtle.importKey("raw", binaryPassphrase, {
        name: "PBKDF2",
    ...
    // Output the "raw" key
    { name: "AES-GCM", length: 256 }, true, // extractable
...
async function deriveKey(passphrase, pbkdf2Salt, hkdfSalt) {
    ...
    { name: "AES-GCM", length: 256 }, false, ["encrypt", "decrypt"] // The chunk key is used only for encryption/decryption
    );
    return chunkKey;
}
/**
 ...
 */
let _sessionPBKDFSalt;
function getSessionPBKDFSalt(refresh = false) {
 ...
 */
async function encryptWithEphemeralSaltBinary(input, passphrase, refresh = false) {
 ...
 */
async function encryptWithEphemeralSalt(input, passphrase, refresh = false) {
 ...
 */
async function decryptWithEphemeralSaltBinary(input, passphrase) {
...
export { HKDF_ENCRYPTED_PREFIX, HKDF_SALTED_ENCRYPTED_PREFIX, createPBKDF2Salt, decrypt, decryptBinary, decryptWithEphemeralSalt, decryptWithEphemeralSaltBinary, encrypt, encryptBinary, encryptWithEphemeralSalt, encryptWithEphemeralSaltBinary, testEncryptionFeature };
