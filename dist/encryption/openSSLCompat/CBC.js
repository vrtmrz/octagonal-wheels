...
const SALT_PREFIX = new Uint8Array([0x53, 0x61, 0x6c, 0x74, 0x65, 0x64, 0x5f, 0x5f]); // "Salted__"
const SALT_LENGTH = 8; // 8 bytes for salt
async function passphraseToKeyMaterial(passphrase) {
    const keyMaterial = await crypto.subtle.importKey("raw", new TextEncoder().encode(passphrase), { name: "PBKDF2" }, false, ["deriveBits"]);
    return keyMaterial;
}
async function generateIV(keyMaterial, salt, iterations = 100000) {
    ...
    const keyIv = await crypto.subtle.deriveBits({
        ...
        iterations,
        hash: "SHA-256",
    }, keyMaterial, length * 8 // deriveBits length is in bits
    );
    return keyIv;
}
async function generateOpenSSLCompatIvKey(passphrase, salt, iterations = 100000, usage = "decrypt") {
 ...
 */
async function encryptCBC(plaintext, passphrase, iterations = 100000) {
 ...
 */
async function decryptCBC(encryptedData, passphrase, iterations = 100000) {
...
export { SALT_LENGTH, SALT_PREFIX, decryptCBC, encryptCBC, generateIV, generateOpenSSLCompatIvKey, passphraseToKeyMaterial };
